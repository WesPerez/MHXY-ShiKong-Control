...
- readiness 会把计划态流程和输入链路分开显示。任务没有缺图片、缺坐标或缺 OCR 文本时，仍可能因为 `condition`、`restore` 或 `onFail=restore` 显示“计划态/恢复计划”；这表示输入链路可演练，但流程分支或恢复序列尚未真实调度。
...
- `onFail=restore` 当前只保留为计划态配置，UI 会标记为“恢复计划”；运行器尚不会自动跳转到恢复步骤，因此失败仍按停止处理。后续 schema v7 需要通过显式 `recoveryStepId` 或恢复流程实现。
