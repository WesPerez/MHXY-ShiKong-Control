...
任务库里的“导入示例包”只补齐内置 5 个示例任务中当前 workspace 缺失的固定 `workflow.id`，并同步生成识别目标占位；它不会覆盖同 id 已存在的用户任务，也不会修改窗口队列。
...
这些样例覆盖 hotkey、图像等待、图像点击、OCR 确认、后台点击、延迟、条件、重试、截图记录、恢复状态。文本输入作为独立步骤类型和可插入片段提供，默认样例不会自动向聊天框或输入框发送文字。它们用于验证模型覆盖面和 UI 操作流，不代表已经可以真实接管游戏。
