...
Ctrl+V 粘贴图片是编辑器资产入口，不是运行时步骤。粘贴后会生成 `Asset` 并绑定到当前 `image_click` / `wait_image` / `detect_page` 步骤，避免后台执行时出现无法运行的伪步骤。
...
原始 `target/command/expect` 仍保留为兼容入口；后续迁移到 `params` / `targets[]` 时应继续保证旧字段可导入。
...
- 后台运行会做更严格的前端校验：未实现的 OCR、缺图片资产的图像步骤、缺坐标/ROI 的点击步骤会阻止执行；观察运行仍允许这些抽象样例通过日志演练。
