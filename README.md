...
- 界面包含目标窗口列表、窗口分配、任务库、步骤时间线、步骤属性、识别目标、预览/ROI、运行队列和运行记录。
- 工作区保存到 Tauri AppData 下的 `workspace.json`，包含 workflows、assignments、assets、runHistory。
- 首次启动会生成 5 个示例任务，每个 10 步以上，覆盖 hotkey、图像等待、图像点击、OCR 确认、后台点击、延迟、条件、重试、粘贴图片、截图记录、恢复状态。
- 已支持把当前任务分配给多个窗口，并对已选窗口执行 dry-run 队列。
- dry-run 只模拟每个 hwnd 的任务会话、互斥锁、步骤进度和日志，不向游戏窗口发送输入。
- 当前不内置真实游戏输入派发器；后续要恢复执行层时必须走 hwnd 后台消息，并重新审计。
...
- 同一个 hwnd 只能有一个运行会话；不同 hwnd 可以独立并行。
...
- 后续恢复真实执行层时，点击、文本、快捷键只能通过目标 hwnd 的后台消息路径实现。
...
2. 补共享 `restore` 恢复流程。
3. 补后台 hwnd 输入执行器，只允许安全消息路径，并让审计脚本覆盖它。
4. 接入图片匹配和 OCR 实测，每补一个真实任务都保留 dry-run、运行报告和输入安全审计。
