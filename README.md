...
- 步骤时间线支持一键插入常用片段，包括打开界面、识图点击、物品右键、状态检查和 10 步完整任务骨架；插入时会同步创建可复用逻辑目标。
...
- Ctrl+V 图片或 ROI 生成目标时，如果当前步骤不适合绑定图片，会自动在当前步骤下方新建可执行步骤，避免误改延迟/热键等步骤语义。
- 后台 `delay` 使用任务里设置的真实等待时长；`retry_until` 对绑定图像/ROI 执行轻量等待循环，不发送额外输入，纯状态型目标会保持明确的计划态等待。
